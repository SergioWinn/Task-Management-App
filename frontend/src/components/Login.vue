<template>
  <div class="flex items-center justify-center min-h-[80vh]">
    ...
    <div class="w-full max-w-sm bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden transition-all hover:shadow-2xl duration-300">
      ...
      <form @submit.prevent="handleLogin" class="px-8 pb-10 space-y-5">
        ...
        <div class="space-y-4">
          <div>
            <label class="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5 ml-1">Username</label>
            <input 
              v-model="username" 
              type="text" 
              required 
              placeholder="admin"
              class="w-full px-5 py-3 bg-gray-50 border border-transparent rounded-xl focus:bg-white focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 outline-none transition-all font-medium text-gray-700 placeholder-gray-400"
            >
          </div>
        ...
        <div v-if="error" class="flex items-center gap-2 p-3 text-sm text-red-600 bg-red-50 rounded-lg animate-pulse">
